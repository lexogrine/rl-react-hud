...
const TeamBox = (props: Props) => {
  ...
  const createPlayerBox = (player: any) => {
    ...
    const foundEventSpec = eventWhitelist.find(
      (e) =>
    ...
    return (
      <div className={"player-box " + side} key={player.id}>
        {avatars[player.steamid]?.url ? (
          <div
            className="avatar"
            style={{
              backgroundImage: `url('${avatars[player.steamid]?.url}')`,
            }}
          >
        ...
        <div
          className={`data-outer-container ${visibleEvent ? "hide" : "show"}`}
        >
          <div className="name-box">
            ...
            <span className={`name ${player.isSonic ? "shake" : ""}`}>
              {player.name}
            </span>
          </div>
          <div className="boost-meter">
            <div
              className="grey"
              style={{ backgroundImage: `url('images/boost/grey.svg')` }}
            />
            <div
              className="fill"
              style={{
                backgroundImage: `url('images/boost/fill.svg')`,
                width: `${player.boost || 0}%`,
              }}
            />
          </div>
        ...
        <div
          ...
          style={{
            backgroundImage: `url('images/eventIcons/${icon}.png')`,
        ...
        {!visibleEvent &&
          lastBallHit &&
          lastBallHit.player.id === player.id && (
